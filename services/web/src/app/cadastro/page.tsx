...
export default function CadastroPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', cpf: '', password: '', gender: 'FEMALE' as 'MALE' | 'FEMALE' });
  ...
  return <main className="mx-auto max-w-2xl px-5 py-12"><Panel className="p-7 md:p-10"><p className="text-xs font-black uppercase tracking-[0.2em] text-lime-700">Comece agora</p><h1 className="mt-2 font-display text-4xl font-bold">Crie sua conta</h1><p className="mt-3 text-sm leading-6 text-ink/55">Tenha seu acompanhamento personalizado no painel.</p><form onSubmit={submit} className="mt-8 space-y-5"><label className="block"><span className="field-label">Nome completo</span><input className="field-control" required minLength={3} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label><label className="block"><span className="field-label">E-mail</span><input className="field-control" type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label><label className="block"><span className="field-label">CPF</span><input className="field-control" required inputMode="numeric" placeholder="000.000.000-00" value={form.cpf} onChange={e => setForm({ ...form, cpf: formatCpf(e.target.value) })} /></label><fieldset><legend className="field-label">Sexo biológico</legend><div className="mt-2 grid grid-cols-2 gap-3">{([{ value: 'FEMALE', label: 'Feminino' }, { value: 'MALE', label: 'Masculino' }] as const).map(option => <label key={option.value} className={`cursor-pointer rounded-2xl border p-4 text-center font-bold ${form.gender === option.value ? 'border-lime-600 bg-lime-50' : 'border-ink/10'}`}><input className="sr-only" type="radio" checked={form.gender === option.value} onChange={() => setForm({ ...form, gender: option.value })} />{option.label}</label>)}</div></fieldset><label className="block"><span className="field-label">Senha</span><input className="field-control" type="password" minLength={8} required value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></label>{error ? <Notice kind="error">{error}</Notice> : null}<Button type="submit" loading={loading} className="w-full">Criar minha conta</Button><p className="text-center text-sm text-ink/50">Já possui conta? <a className="font-bold text-lime-700" href="/login">Entrar</a></p></form></Panel></main>;
}
