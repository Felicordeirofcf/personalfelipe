...
export default function AlunoPage() {
  ...
  const currentStreak = (() => { let streak = 0; const cursor = new Date(); while (sessionDays.has(cursor.toISOString().slice(0, 10))) { streak += 1; cursor.setDate(cursor.getDate() - 1); } return streak; })();
  const weekDays = Array.from({ length: 7 }, (_, index) => { const date = new Date(); date.setDate(date.getDate() - (6 - index)); return date; });
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[1.2fr_0.8fr]">
        <Panel className="p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-emerald-300">Constância semanal</p><h2 className="mt-1 font-display text-2xl font-bold text-white">Seu calendário de presença</h2></div><CalendarDays className="text-emerald-300" /></div><div className="mt-6 grid grid-cols-7 gap-2">{weekDays.map((date) => { const key = date.toISOString().slice(0, 10); const done = sessionDays.has(key); return <div key={key} className="text-center"><p className="text-[10px] font-bold uppercase text-zinc-500">{date.toLocaleDateString('pt-BR', { weekday: 'short' }).replace('.', '')}</p><div className={`mx-auto mt-2 grid h-11 w-11 place-items-center rounded-2xl border ${done ? 'border-emerald-400 bg-emerald-500 text-zinc-950 shadow-lg shadow-emerald-500/20' : 'border-zinc-800 bg-zinc-900 text-zinc-500'}`}><span className="text-sm font-black">{date.getDate()}</span></div>{done ? <CheckCircle2 size={13} className="mx-auto mt-1 text-emerald-300" /> : <span className="mt-1 block h-3" />}</div>; })}</div><div className="mt-6 space-y-2">{sessions.slice(0, 5).map((session) => <div key={session.id} className="flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 py-2.5"><span className="text-sm font-bold text-zinc-200">{session.dayTitle}</span><span className="text-xs text-zinc-500">{new Date(session.completedAt).toLocaleDateString('pt-BR')}</span></div>)}{sessions.length === 0 ? <p className="text-sm text-zinc-400">Conclua seu primeiro treino para iniciar o calendário.</p> : null}</div></Panel>

        <Panel className="p-6"><p className="text-xs font-black uppercase tracking-wider text-emerald-300">Gamificação</p><h2 className="mt-1 font-display text-2xl font-bold text-white">Evolução de presença</h2><div className="mt-6 grid gap-3"><div className="flex items-center gap-3 rounded-2xl border border-orange-500/20 bg-orange-950/20 p-4"><Flame className="text-orange-300" /><div><p className="text-xs font-bold text-zinc-400">Sequência atual</p><p className="text-2xl font-black text-white">{currentStreak} {currentStreak === 1 ? 'dia' : 'dias'}</p></div></div><div className="flex items-center gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-950/20 p-4"><Trophy className="text-emerald-300" /><div><p className="text-xs font-bold text-zinc-400">Treinos concluídos</p><p className="text-2xl font-black text-white">{sessions.length}</p></div></div></div><p className="mt-5 text-sm leading-6 text-zinc-400">Cada treino concluído fortalece sua constância. Mantenha a sequência e acompanhe sua evolução semanal.</p></Panel>
