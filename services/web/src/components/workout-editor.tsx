                ...
                <tbody>
                  {split.exercises.map((exercise, exerciseIndex) => (
                    <tr key={exercise.id ?? `new-${exerciseIndex}`} className="border-b border-zinc-800/50 bg-transparent align-top last:border-0 hover:bg-zinc-800/30">
                      ...
                      <td className="p-2"><div className="flex w-56 items-center gap-1.5"><input type="url" className={`${editorInput} min-w-0 flex-1 text-xs`} value={exercise.videoUrl ?? ''} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'videoUrl', event.target.value)} placeholder="https://..." />{exercise.videoUrl ? <a href={exercise.videoUrl} target="_blank" rel="noreferrer" className="inline-flex shrink-0 items-center gap-1 rounded-md border border-rose-500/30 bg-rose-500/10 px-2.5 py-1 text-xs font-semibold text-rose-400 transition hover:bg-rose-500/20" title={`Abrir vídeo de ${exercise.name}`}><Youtube size={14} /> Vídeo</a> : null}</div></td>
