...
export function WorkoutEditor({ workout, onChange }: { workout: Workout; onChange: (workout: Workout) => void }) {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col justify-between gap-4 border-b border-ink/10 pb-5 md:flex-row md:items-center">
        <div>
          <div className="flex items-center gap-3"><StatusBadge status={workout.status} /><span className="text-xs font-bold text-ink/40">Criado em {new Date(workout.createdAt).toLocaleDateString('pt-BR')}</span></div>
      ...
      <label className="block">
        ...
        <textarea value={draft.rationale} disabled={workout.status !== 'DRAFT'} onChange={(event) => setDraft({ ...draft, rationale: event.target.value })} className="field-control min-h-28 resize-y disabled:bg-sand/60" />
      </label>

      <div className="mt-7 space-y-5">
        {draft.splits.map((split, splitIndex) => (
          <section key={split.id ?? `new-${splitIndex}`} className="overflow-hidden rounded-2xl border border-ink/10 bg-[#fbfcf9]">
            <div className="flex flex-col gap-3 border-b border-ink/10 bg-sand/70 p-4 md:flex-row md:items-center">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-ink text-xs font-black text-lime-300">{String(splitIndex + 1).padStart(2, '0')}</span>
              ...
              <input value={split.focus} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateSplit(splitIndex, 'focus', event.target.value)} className="min-w-52 flex-[2] rounded-lg border border-transparent bg-transparent px-2 py-1 text-sm font-semibold text-ink/55 focus:border-lime-500 disabled:opacity-80" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1120px] text-left text-sm">
                <thead className="border-b border-ink/10 bg-white text-[10px] uppercase tracking-wider text-ink/40">
                ...
                <tbody>
                  {split.exercises.map((exercise, exerciseIndex) => (
                    <tr key={exercise.id ?? `new-${exerciseIndex}`} className="border-b border-ink/5 bg-white align-top last:border-0">
                      <td className="p-2 pl-4"><input className="w-48 rounded-lg border border-ink/10 px-2 py-2 font-bold disabled:border-transparent disabled:bg-transparent" value={exercise.name} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'name', event.target.value)} /></td>
                      <td className="p-2"><input type="number" min="1" max="10" className="w-16 rounded-lg border border-ink/10 px-2 py-2 disabled:border-transparent disabled:bg-transparent" value={exercise.sets} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'sets', Number(event.target.value))} /></td>
                      <td className="p-2"><input className="w-24 rounded-lg border border-ink/10 px-2 py-2 disabled:border-transparent disabled:bg-transparent" value={exercise.reps} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'reps', event.target.value)} /></td>
                      <td className="p-2"><input type="number" min="0" max="5" className="w-16 rounded-lg border border-ink/10 px-2 py-2 disabled:border-transparent disabled:bg-transparent" value={exercise.rir} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'rir', Number(event.target.value))} /></td>
                      <td className="p-2"><input type="number" min="15" max="600" className="w-20 rounded-lg border border-ink/10 px-2 py-2 disabled:border-transparent disabled:bg-transparent" value={exercise.restSeconds} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'restSeconds', Number(event.target.value))} /></td>
                      <td className="p-2"><input className="w-24 rounded-lg border border-ink/10 px-2 py-2 disabled:border-transparent disabled:bg-transparent" value={exercise.cadence ?? ''} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'cadence', event.target.value)} /></td>
                      <td className="p-2"><input type="url" className="w-52 rounded-lg border border-ink/10 px-2 py-2 text-xs disabled:border-transparent disabled:bg-transparent" value={exercise.videoUrl ?? ''} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'videoUrl', event.target.value)} placeholder="https://..." /></td>
                      <td className="p-2"><textarea className="min-h-10 w-56 resize-y rounded-lg border border-ink/10 px-2 py-2 text-xs disabled:border-transparent disabled:bg-transparent" value={exercise.notes ?? ''} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'notes', event.target.value)} /></td>
            ...
            {workout.status === 'DRAFT' ? <button onClick={() => addExercise(splitIndex)} className="flex w-full items-center justify-center gap-2 border-t border-dashed border-ink/15 bg-white py-3 text-xs font-extrabold text-lime-700 hover:bg-lime-50"><Plus size={15} /> Adicionar exercício</button> : null}
          </section>
        ))}
      </div>
