  ...
  return (
    <div className="text-zinc-100">
      <div className="mb-6 flex flex-col justify-between gap-4 border-b border-zinc-800 pb-5 md:flex-row md:items-center">
        ...
        {workout.status === 'DRAFT' ? <div className="flex flex-wrap gap-2"><Button variant="ghost" loading={saving} onClick={save}><Save size={17} /> Salvar rascunho</Button><Button loading={approving} onClick={approve}><CheckCircle2 size={17} /> Aprovar treino</Button></div> : null}
      </div>
      ...
      <div className="mt-7 space-y-5">
        {draft.splits.map((split, splitIndex) => (
          <section key={split.id ?? `new-${splitIndex}`} className="overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-950/35 shadow-lg shadow-black/10">
            <div className="flex flex-col gap-3 border-b border-zinc-700 bg-zinc-900/90 p-4 md:flex-row md:items-center">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1120px] text-left text-sm">
                <thead className="border-b border-zinc-800 bg-zinc-950/80 text-[10px] font-bold uppercase tracking-wider text-zinc-400">
                ...
                <tbody>
                  {split.exercises.map((exercise, exerciseIndex) => (
                    <tr key={exercise.id ?? `new-${exerciseIndex}`} className="border-b border-zinc-800 bg-[#131B2E] align-top last:border-0 hover:bg-zinc-900/60">
                      <td className="p-2 pl-4"><input className={`${editorInput} w-48 font-bold`} value={exercise.name} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'name', event.target.value)} /></td>
                      ...
                      <td className="p-2"><input type="url" className={`${editorInput} w-52 text-xs`} value={exercise.videoUrl ?? ''} disabled={workout.status !== 'DRAFT'} onChange={(event) => updateExercise(splitIndex, exerciseIndex, 'videoUrl', event.target.value)} placeholder="https://..." /></td>
